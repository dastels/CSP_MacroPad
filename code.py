...
while True:
    ...
    if position != last_position:
        if len(apps[app_index].macros) < 13:
            continue    # No encoder macros, just resume main loop
        if position < last_position:
            key_number = 13 # else process below as 13th macro
        else:
            key_number = 14
        pressed = True
    ...
    elif encoder_switch != last_encoder_switch:
    ...
    if pressed:
        print("Pressing {0}".format(apps[app_index].macros[key_number][1]))
